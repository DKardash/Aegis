...
Aegis - утилита для автоматизации запуска стресс-тестов, сервисных проверок ПК, обновления NVIDIA-драйверов и Zapret. Программа запускает внешние приложения по аргументам командной строки, выполняет действия в нужной последовательности, может отправлять скриншот экрана в Telegram и вести отдельные журналы работы.
...
- Автоматизация OCCT для запуска стресс-тестов CPU, GPU и Memory.
...
- Автоматическое скачивание и обновление `Flowseal/zapret-discord-youtube` до последней версии.
- Отложенная отправка скриншота экрана в Telegram с именем ПК и локальным IP-адресом.
- Редактирование путей до программ в `settings.config`. Если файл отсутствует - он создатся после первого запуска программы.
...
| `/OCCT` | Запустить OCCT с выбранными стресс-тестами. |
...
| `Zapret.Update` | Скачать и установить последний релиз Flowseal Zapret. |
...
Команда `/OCCT` поддерживает следующие параметры:

| Параметр | Назначение |
| --- | --- |
| `-CPU` | Стресс-тест процессора. |
| `-GPU` | Стресс-тест видеокарты. |
...
<details>
<summary><strong>Обновление Flowseal Zapret</strong></summary>

Команда `Zapret.Update` получает последний релиз `Flowseal/zapret-discord-youtube` через GitHub API, выбирает ZIP-архив и устанавливает его в каталог, указанный в `settings.config`.

Пример:

```bat
"C:\Gizmo Utilities\Aegis\Aegis.exe" Zapret.Update
```
</details>

> [!WARNING]
> `Zapret.Update` полностью заменяет содержимое настроенного каталога Zapret. Пользовательские файлы рекомендуется хранить отдельно. Если файлы Zapret заняты запущенным процессом или службой, обновление может завершиться ошибкой.
